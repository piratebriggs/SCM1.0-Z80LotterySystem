...
; Global constants - RC2014 module hardware
...
#INCLUDE    Hardware\SC_S2\BankedRAM.asm
#INCLUDE    Hardware\SC_S2\SerialBB.asm
...
; Initialise SC114 bit-bang serial port as device 6
            CALL SC114_Serial_Initialise
            LD   HL,@PtrSerial  ;Pointer to vector list
            LD   B,2            ;Number of jump vectors
            LD   A,kFnDev6In    ;First device jump entry
...
; Test if any console devices have been found
            LD   HL,iHwFlags    ;Get hardware flags
            LD   A,(HL)         ;Get device detected flags
            SET  3,(HL)         ;Set bit-bang port present flag
            OR   A              ;Any other serial ports found?
            RET  NZ             ;Yes, so return
; Failed to find UART so initialise bit-bang port as device 1
            LD   HL,@PtrSerial  ;Pointer to vector list
            LD   B,2            ;Number of jump vectors
            LD   A,kFnDev1In    ;First device jump entry
            CALL InitJumps      ;Set up serial vectors
; Indicate failure by flashing the motherboard LED
            LD   DE,200
            CALL Delay
            LD   A,1
...
@PtrSerial: ; Device #6 = Serial bit-bang SC114
            .DW  SC114_Serial_InputChar
            .DW  SC114_Serial_OutputChar
...
@szHardware:
            .DB  "SC114 motherboard",kNewLine,kNull
...
Hardware_Devices:
            ...
            CALL NZ,OutputZString ;Yes, so list it
            LD   DE,@szHwBB     ;Bit-bang serial message
            JP   OutputZString  ;Yes, so list it
            ;RET
...
@szHwBB:    .DB  "6 = Bit-bang serial(@28)",kNewLine,kNull
...
; Bit 1 = Serial Z80 SIO   #1 detected
; Bit 2 = Serial 6850 ACIA #2 detected
; Bit 3 = Serial bit-bang port detected
...
; **********************************************************************
; **  End of Hardware manager for SC_S2                               **
; **********************************************************************
